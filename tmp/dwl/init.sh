...
if [ "`find ${APACHE_SSL_DIR} -type d -name "${DWL_USER_DNS}" | wc -l`" = "0" ]; then
    ...
    sed -i "s|# SSLCertificateFile|${APACHE_SSL_DIR}/${DWL_USER_DNS}/apache.crt|g" /etc/apache2/sites-available/${DWL_USER_APACHE_CONF}.conf;
    sed -i "s|# SSLCertificateKeyFile|${APACHE_SSL_DIR}/${DWL_USER_DNS}/apache.key|g" /etc/apache2/sites-available/${DWL_USER_APACHE_CONF}.conf;
fi
