...
if [ "`find ${APACHE_SSL_DIR} -type d -name "${DWL_USER_DNS}" | wc -l`" = "0" ]; then
    ...
    cp -f /etc/apache2/sites-enabled/virtualhost.conf.origin /etc/apache2/sites-enabled/virtualhost.conf;
    sed -i "s|# ServerName|ServerName ${DWL_USER_DNS}|g" /etc/apache2/sites-enabled/virtualhost.conf;
    sed -i "s|# SSLCertificateFile|SSLCertificateFile ${APACHE_SSL_DIR}/${DWL_USER_DNS}/apache.crt|g" /etc/apache2/sites-enabled/virtualhost.conf;
    sed -i "s|# SSLCertificateKeyFile|SSLCertificateKeyFile ${APACHE_SSL_DIR}/${DWL_USER_DNS}/apache.key|g" /etc/apache2/sites-enabled/virtualhost.conf;
fi
