for conf in `find /etc/apache2/sites-available -type f -name "*.conf"`; do
  ...
  if [ "`find ${APACHE_SSL_DIR}/${DWL_USER_DNS} -type f | wc -l`" = "0" ]; then
    ...
    if [ "$((`echo ${DWL_USER_DNS} | sed 's/[^\.]//g' | wc -c` -1 ))" != 1 ]; then
      ...
      sed -i "s|# ServerName|ServerName `echo ${DWL_USER_DNS_SERVERNAME}`|g" /etc/apache2/sites-available/${DWL_USER_DNS}.conf;
      sed -i "s|# ServerAlias|ServerAlias `echo ${DWL_USER_DNS}`|g" /etc/apache2/sites-available/${DWL_USER_DNS}.conf;
      sed -i "s|# SSLCertificateFile|SSLCertificateFile `echo ${APACHE_SSL_DIR}/${DWL_USER_DNS_SERVERNAME}`/apache.crt|g" /etc/apache2/sites-available/${DWL_USER_DNS_SERVERNAME}.conf;
      sed -i "s|# SSLCertificateKeyFile|SSLCertificateKeyFile `echo ${APACHE_SSL_DIR}/${DWL_USER_DNS_SERVERNAME}`/apache.key|g" /etc/apache2/sites-available/${DWL_USER_DNS_SERVERNAME}.conf;
    else
